...
class BertweetModel(pl.LightningModule):
    """
    BERTweet model specifically for tweet classification.
    This is the same as BertweetClassifier but with BERTweet as the default model.
    """
    def __init__(
        self,
        transformer_model_name: str = "vinai/bertweet-base",
        ...
        # 1) encoder - Using BERTweet specifically
        ...
        # 4) loss
        cls_w = None if class_weights is None else class_weights.to(self.device)
        self.loss_fn = (
            nn.BCEWithLogitsLoss(weight=cls_w)
            if use_soft_labels
    ...
    def forward(self, texts: list[str]):
        ...
        # pool: either use [CLS] or mean‐pool last hidden states
        # for RoBERTa‐style models, CLS is at position 0:
