...
def classify_tweet_direct_with_explanation(text, model_name, api_key=MARTIAN_API_KEY):
    """
    Directly classify a tweet as 1, 2, or 3 with explanation
    """
...
Respond with ONLY the number (1, 2, or 3) corresponding to the category, followed by a brief explanation.
Format your response as: NUMBER [explanation]
For example: 1 [This tweet focuses on describing a problem with healthcare costs]
...
Tweet: {text}"""
    ...
    try:
        ...
        # Parse the response format: NUMBER [explanation]
        try:
            # Extract the number
            number_part = prediction_text.split()[0]
            pred = int(number_part)
            
            # Extract explanation (everything after the number)
            if '[' in prediction_text:
                explanation = prediction_text.split('[', 1)[1].rstrip(']')
            else:
                explanation = ' '.join(prediction_text.split()[1:])
...
def test_direct_classification_with_explanation(model_name, input_csv, output_csv):
    # Load the input data
    ...
    for idx, row in df.iterrows():
        ...
        result = classify_tweet_direct_with_explanation(text, model_name)
        
        if result[0] is not None:
            predictions.append(result[0])
            explanations.append(result[1])
            valid_indices.append(idx)
            print(f"Row {idx+1}/{total_rows}: Predicted: {result[0]} - {result[1]}")
        else:
    ...
    output_df['explanation'] = None
    
    # Fill in the predictions and explanations for valid predictions
    for i, idx in enumerate(valid_indices):
        output_df.loc[idx, 'predicted_class'] = predictions[i]
        output_df.loc[idx, 'explanation'] = explanations[i]
...
def main():
    parser = argparse.ArgumentParser(description='Test direct classification with explanation')
    ...
    parser.add_argument('--output_csv', type=str, default='direct_classification_with_explanation_results.csv')
    ...
    # Run the test
    result_df = test_direct_classification_with_explanation(args.model, args.input_csv, args.output_csv)
