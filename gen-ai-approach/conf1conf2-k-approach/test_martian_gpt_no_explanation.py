...
def classify_tweet_without_explanation(text, model_name, api_key=MARTIAN_API_KEY):
    """
    Classify a single tweet and get confidence scores for classes 1 and 2 without explanation
    """
...
Format your response as: CONF1,CONF2
For example: 85,10
...
def test_martian_no_explanation_on_csv(model_name, input_csv, output_csv):
    """
    Test Martian GPT on CSV and output results with confidence scores without explanations
    """
    # Load the input data
    ...
    for idx, row in df.iterrows():
        ...
        result = classify_tweet_without_explanation(text, model_name)
        
        if result[0] is not None:
            conf1_scores.append(result[0])
            conf2_scores.append(result[1])
            valid_indices.append(idx)
            print(f"Row {idx+1}/{total_rows}: Class 1: {result[0]}, Class 2: {result[1]}")
        else:
            print(f"Row {idx+1}/{total_rows}: Failed to get prediction")
        
        # Add delay to avoid rate limiting from Martian API
        time.sleep(0.5)
...
def main():
    parser = argparse.ArgumentParser(description='Test Martian GPT with confidence scores without explanations')
    ...
    parser.add_argument('--output_csv', type=str, default='martian_confidence_GOLD_TEST_no_explanation_full.csv')
    ...
    # Run the test
    result_df = test_martian_no_explanation_on_csv(args.model, args.input_csv, args.output_csv)
