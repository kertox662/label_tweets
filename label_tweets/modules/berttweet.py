...
class TweetsTVTDataModule(pl.LightningDataModule):
    ...
    def read_csv(self, remove_disagreements: bool = False):
        ...
        data_df['AR'] = data_df['AR'].replace({4: 2})
        data_df['MB'] = data_df['MB'].replace({4: 2})
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        # Handle test split (skip if test_size is 0)
        if self.test_size > 0:
            ...
            # No test split - use all data for training/validation
            modeldev_df = df[[self.text_col, self.label_col]]
            test_df = df[[self.text_col, self.label_col]].iloc[:0]  # Empty dataframe with correct columns
        
        # Split into train and validation
        train_df, val_df = train_test_split(
            modeldev_df,
            test_size=self.val_size / (1 - self.test_size) if self.test_size > 0 else self.val_size,
        ...
        # Labels in CSV are 1-indexed [1,2,3], need to count them correctly for 0-indexed classes [0,1,2]
...
class BertweetModule(pl.LightningModule):
    ...
    def __init__(self,
        ...
        # Put model in training mode immediately
        self.model.train()
    ...
    def on_train_epoch_start(self):
        """Ensure model stays in training mode each epoch"""
        self.model.train()
    ...
    def configure_optimizers(self):
        ...
        if self.total_steps_override is not None:
            ...
            # Fallback: return optimizer only
            return optimizer
